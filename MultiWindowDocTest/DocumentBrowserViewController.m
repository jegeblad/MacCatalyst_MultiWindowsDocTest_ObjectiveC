...
- (void)viewDidLoad {
    [super viewDidLoad];
	NSLog(@"DocumentBrowserViewController.viewDidLoad");
	visibleDocVC = nil;
    self.delegate = self;
...
-(void) viewDidAppear:(BOOL)animated
{
	...
	NSUserActivity * userActivity = [[NSUserActivity alloc] initWithActivityType:@"com.mexircus.docbrowser"];
	userActivity.userInfo = [NSDictionary dictionaryWithObject:@"browser" forKey:@"browser"];
...
-(void) viewDidDisappear:(BOOL)animated
{
	...
	NSLog(@"Doc browser %@ did disappear retain count", self);
	activeOnScene = nil;
}
...
-(void) dealloc
{
	NSLog(@"Doc browser %@ dealloc", self);
}
...
- (void)presentDocumentAtURL:(NSURL *)documentURL
{
	...
	[documentViewController.document openWithCompletionHandler:^(BOOL success)
	{
		if (success)
		{
//			[[RecentDocumentHandler sharedHandler] addBookmarkedURLForURL:documentURL];
